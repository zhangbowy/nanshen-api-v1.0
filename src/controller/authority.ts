...
module.exports = class extends Base {
     ...
     * @return roleDetail
     */
    async roleDetailAction(): Promise<any> {
        try {
     ...
     */
    async addRoleAction(): Promise<any> {
        try {
     ...
     * @return boolean
     */
    async editRoleAction(): Promise<any> {
        try {
     ...
     * @return boolean
     */
    async delRoleAction(): Promise<any> {
        try {
            const admin_role_id = this.post('admin_role_id');
            const shop_id = this.ctx.state.admin_info.shop_id;
    ...
    async adminListAction(): Promise<void> {
        try {
            const page: number = this.post('currentPage') || think.config('currentPage');
            const limit: number = this.post('pageSize') || think.config('pageSize');
            const shop_id = this.ctx.state.admin_info.shop_id;
     ...
     * @return boolean
     */
    async addAdminAction(): Promise<any> {
        try {
     ...
     * @return boolean
     */
    async editAdminAction(): Promise<any> {
        try {
     ...
     * @param {id} 管理员id
     */
    async delAdminAction(): Promise<any> {
        try {
            const id = this.post('id');
            const shop_id = this.ctx.state.admin_info.shop_id;
