...
export default class extends Base {
  ...
  async indexAction() {
    try {
      ...
      return await this.display('error/404.html');
      // const vid =  this.get('vid');
      // const url = `http://vv.video.qq.com/getinfo?vids=${vid}&platform=101001&charge=0&otype=json`;
      // const options = {
      //   method: 'GET',
      //   url,ssh://git@git.66ip.net:10022/zhangbo/bo-api-v1.0.git
      //   header: {
      //     'X-Forwarded-For': '47.100.37.157',
      //     'client-ip': '47.100.37.157'
      //   },
      //   json: true
      //   // qs: {
      ...
      //   // }
      // };
      // const reg = /QZOutputJson=(.*);/;
      // const $data = await rp(options);
      // const data = JSON.parse(reg.exec($data)[1])
      // const v = data.vl.vi['0'].fn;
      // const vkey = data.vl.vi['0'].fvkey;
      // const list = [];
      // for (const item of data.vl.vi['0'].ul.ui) {
      //   list.push(item.url + v + '?vkey=' + vkey );
      // }
      // // const result = JSON.parse($data.split('=')[1])
      // this.$success(list);
    } catch (e) {
      this.dealErr(e);
    }
      ...
      if (this.header("ops")) {
        ...
        if (r_sign == sign) {
          ...
          for (const orderInfo of orderList) {
            ...
            const dst_Buffer: any = await this.getBuffer(this, order_item.order_dst_path, true);
            ...
            if (!order_item.order_txt_file_path) {
              ...
              // @ts-ignore
              const res = await designController.getDesignInfo(order_item.order_emb_path);
  ...
  async getLangAction() {
    try {
      const platform = this.get('platform') || 0;
      const keywords = this.get('keywords') || '';
      const is_func_name = this.get('is_func_name') || 0;
      const page: number = this.get('currentPage') || think.config('currentPage');
      const limit: number = this.get('pageSize') || think.config('pageSize');
      const where: any  = {'cn|en': ['LIKE', `%${keywords}%`]};
      if (is_func_name) {
        if (is_func_name == 'true') {
          where.is_func_name = 1;
        } else {
          where.is_func_name = 0;
        }
      }
      if (platform) {
        where.platform = ['IN', [platform, 0]];
      }
      const res = await this.model('lang').where(where).page(page, limit).countSelect();
      return this.success(res, '多语言列表!');
    } catch (e) {
      this.dealErr(e);
    }
  }
