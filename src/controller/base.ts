...
export default class extends restController {
  ...
  async __before() {
    try {
      ...
      const whiteList = ['/designer/getLang', '/index/getLang', '/getLang', '/recharge/notify', '/index/index.html', '/', '/order/exportExcel', '/admin/login', '/admin/getCaptcha', '/admin/logOut', '/index/downLoad', '/index/getDst'];
      // if (whiteList.indexOf(this.ctx.path) === -1) {
      //   const sign1 = await this.cookie('admin-token');
      //   console.log(sign1, 'sign1');
      //   const adm_sign = sign1  ||  this.header("adm_sign");
      ...
      //   }
      //   const admin = await this.model('admin').where({id: admin_id, del: 0}).find();
      ...
      //      * 管理后台登录用户信息
      //      */
      //     this.ctx.state.admin_info = admin;
      //   } else {
